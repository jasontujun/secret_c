...
void test_mem(const char *rw_inname, const char *rw_outname) {
    ...
    myse->data = str;
    myse->size = str_size;
    int w_result = write_image_directly(rw_inname, rw_outname, myse, myse->size);
    ...
    se->size = str_size;
    int r_result = read_image_directly(rw_outname, se);
...
void test_file(const char *rw_inname, const char *rw_outname) {
    ...
    int w_result = write_image_directly(rw_inname, rw_outname, myse, 0);
